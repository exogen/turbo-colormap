import { KDTree } from "kdtree-typescript";

export type Color = ArrayLike<number>;

const colormapFloatArray = new Float32Array([
...
const colormapIntArray = new Uint8ClampedArray(
  colormapFloatArray.map((value) => normalizedToIntensity(value))
);
 ...
 */
export const colormap: Array<Float32Array> = new Array(256);
for (let i = 0; i < colormap.length; i++) {
  const flatIndex = i * 3;
  colormap[i] = colormapFloatArray.subarray(flatIndex, flatIndex + 3);
}
...
for (let i = 0; i < rgbColormap.length; i++) {
  const flatIndex = i * 3;
  rgbColormap[i] = colormapIntArray.subarray(flatIndex, flatIndex + 3);
}
...
export function rgbToIntensity(
  ...
  // @ts-expect-error
  const bestValue = rgbTree.knnSearch(rgbColor, 1)[0];
  if (cache) {
